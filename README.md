...
Run `./moar_dots.sh` from the repo root with one of the subcommand flags below. Pass `-h` or `--help` to see all available options at any time.
...
-u  --uninstall      Uninstall an existing version of moar_dotz
```
...
./moar_dots.sh --restore

# Point at a specific backup
./moar_dots.sh --restore ~/.config/dots/backup/20250101-09:00:00.tar.gz
```
